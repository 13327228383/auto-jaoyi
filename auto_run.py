...
CFG = {
    ...
    "MAX_DAILY_LOSS_PCT": 0.05,    # 单日组合亏损超 5% 暂停当日再平衡（安全闸）。
    #   回测 research/backtest_joint_sweep.py（复权干净数据、trail2%+MIN_HOLD=3+冷却2+迟滞带+T+1+滑点0.1%+万2.38 联合网格）：
    #   halt 5%~6% 为平稳最高平台（单维样本外Calmar 1.90 / 1.88，off 回撤-33.87% 最差）；
    #   联合 Top：trail2%/halt5%/mhold3 全期年化24.68%、回撤-20.45%、Calmar1.21、样本外年化63.74%、Calmar3.42
    #   （halt6% 与5% 几乎持平 24.27%/Calmar1.20，5% 联合最高点）→ 采纳 5%。
    #   （注：历史 3%/5%/6% 横跳是未除权复权脏数据所致，本数据已统一前复权口径。）
    ...
    "SR_TRAIL_PCT": 0.02,         # 跟踪回撤阈值：从持仓最高价回撤 2% 即离场，只升不降（普通标的+防御资产统一。
    #   注意：此值为【代码默认】；运行时以 DB active_param 现役权威 0.02 热更覆盖（DB 与代码已对齐一致的 2%）
    # —— 防御资产(黄金518880) 高点回落卖出：防御黄金需锁定高点利润，防冲高回落——
    #   旧版对 DEFENSIVE 明确排除跟踪止损（code != CFG['DEFENSIVE']），黄金冲高获利后利润守不住，
    #   例：某日盘中盈利 900+ → 回落只剩 500+ 未锁定。
    "DEF_PEAK_STOP": 0.02,        # 防御资产专用：从持仓以来最高价回撤 2% 即离场（值与 SR_TRAIL_PCT 统一；
    #   与 DB active_param 现役权威 0.02 一致；运行时如热更覆盖则按现役值）
    ...
    # 此处固定为代码常量。回测 research/backtest_joint_sweep.py（复权干净数据、含滑点+万2.38+冷却2+迟滞带+T+1）
    # 联合网格结果：mhold 3>5>0（全期年化 17.08%/15.32%/13.13%、样本外 42.51%/37.85%/32.02%）→ 采纳 3。
    "MIN_HOLD_DAYS": 3,
