    ...
    try:
        ...
        with c.cursor() as cur:
            ...
            cur.execute("""CREATE TABLE IF NOT EXISTS trade_log(
                ...
                created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                KEY idx_ts (ts), KEY idx_param (param_set_id)
            ) ENGINE=InnoDB""")
    ...
    try:
        ...
        with c.cursor() as cur:
            cur.execute("SELECT id FROM param_sets WHERE SLOPE_WINDOW=%s AND SR_TRAIL_PCT=%s AND "
                        "MIN_HOLD_DAYS=%s AND DEF_MOM_DAYS=%s AND DEF_MOM_ENTER=%s AND DEF_MOM_EXIT=%s AND HOLD_N=%s",
                        (row.get("SLOPE_WINDOW"), row.get("SR_TRAIL_PCT"), row.get("MIN_HOLD_DAYS", 5),
...
def insert_trade(rec, param_set_id=None):
    ...
    try:
        c = connect(DB_NAME)
        with c.cursor() as cur:
            cur.execute("INSERT INTO trade_log(ts,side,code,price,qty,amount,reason,param_set_id) "
                        "VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
                        (rec["ts"], rec["side"], rec["code"], rec.get("price"), rec.get("qty"),
                         rec.get("amount"), rec.get("reason"), param_set_id))
        return True
    except Exception:
        return False
    finally:
...
def real_trades(param_set_id=None):
    """读取 trade_log（可按参数组过滤）。返回 list[dict]。"""
    if not ensure_db():
        return []
    try:
        c = connect(DB_NAME)
        with c.cursor() as cur:
            if param_set_id is not None:
                cur.execute("SELECT ts,side,code,price,qty,amount,reason,param_set_id FROM trade_log "
                            "WHERE param_set_id=%s ORDER BY ts", (param_set_id,))
            else:
                cur.execute("SELECT ts,side,code,price,qty,amount,reason,param_set_id FROM trade_log ORDER BY ts")
            cols = ["ts", "side", "code", "price", "qty", "amount", "reason", "param_set_id"]
